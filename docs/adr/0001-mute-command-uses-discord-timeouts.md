...
`MuteCommand` actually rejects anything over 21 days, not Discord's own 28-day limit. `ModUtil.setMuteTimeout` schedules the eventual `UNMUTE` audit-log write with a plain `setTimeout`, whose delay is a 32-bit signed int (~24.85 day max) — a delay past that overflows and fires almost immediately instead of waiting, silently corrupting the audit trail for any mute between ~24.85 and 28 days. Rather than add reschedule-chaining logic to support the full 28 days correctly, `ModUtils.MAX_TIMEOUT_DURATION_SECONDS` is capped at 21 days, a round number safely under the 32-bit ceiling.

The same `setTimeout` constraint applies to temporary bans - `setBanTimeout` has an identical unbounded `setTimeout` call, and `BanCommand` never capped duration at all. `MAX_TIMEOUT_DURATION_SECONDS` is shared (not mute-specific) and applied to `BanCommand`, `SetWarnPunishments`' `ban` entries, and (transitively, since it delegates) `BanRmCommand` too - a specified ban duration over 21 days is rejected the same way as mute's. Unlike mute, an *unspecified* ban duration still means permanent, since Discord bans (unlike timeouts) have a genuine permanent state and don't rely on `setBanTimeout` at all in that case.
