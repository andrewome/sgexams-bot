...
export class CharacterSubstitutor {
    ...
    public convertText(text: string): string[] {
        ...
        const f = (a: any[], b: any[]): any[] =>
            ([] as any).concat(...a.map(a2 => b.map(b2 => ([] as any).concat(a2, b2))));
        ...
        let oneToManyCharsFound: string[] = [];
        ...
        let length = oneToManyCharsFound.length;
        let outputs: string[] = [];
        if(length === 0) {
            outputs.push(output);
            return outputs;
        } else { // Not empty, generate the rest of the contents
            let mappings: string[][] = [];
            let combinations: string[][] = [];
            for(let char of oneToManyCharsFound) {
            ...
            if(length === 1) {
                let charToBeReplaced = oneToManyCharsFound[0];
                let regex: RegExp;
            ...
            for(let i of combinations) {
                let _output = output;
                for(let j in i) {
                    let charToBeReplaced = oneToManyCharsFound[j];
                    let regex: RegExp;
                    if(charSet.has(charToBeReplaced)) {
                        regex = new RegExp(`\\${charToBeReplaced}`, "g");
                    } else {
